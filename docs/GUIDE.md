...
**Architecture:** the bridge lives in `src/whatsapp/`. `handler.ts` is provider-agnostic (accepts `(jid, text, sendReply)` and reuses `runAgent` + memory). Two adapters live alongside: `baileys.ts` (unofficial WebSocket) and `meta.ts` (official Meta Cloud API).

**Switch to Meta Cloud API (once business is verified):** set `WHATSAPP_PROVIDER=meta` + fill in `WHATSAPP_META_ACCESS_TOKEN`, `WHATSAPP_META_PHONE_NUMBER_ID`, `WHATSAPP_META_VERIFY_TOKEN`, `WHATSAPP_META_APP_SECRET`. The bot then starts an HTTP webhook on `WHATSAPP_META_WEBHOOK_PORT` (default 3001, path `/whatsapp/webhook`).

**Meta setup walkthrough:**
1. Verify a business in [Meta Business Manager](https://business.facebook.com). This takes days — plan ahead.
2. Create an app at [developers.facebook.com](https://developers.facebook.com) → add **WhatsApp** product.
3. In the WhatsApp → Getting Started page: copy the **Phone number ID** and generate a **permanent access token** (via System User, not the 24-hour temporary one).
4. Copy the **App Secret** from App Settings → Basic.
5. Invent a random **Verify Token** (any string — this isn't from Meta, you just paste the same one into both .env and Meta's UI).
6. Put an **HTTPS reverse proxy** in front of the bot — Meta will not talk to plain HTTP. Options: Cloudflare Tunnel (free, no domain), nginx + Let's Encrypt (needs a domain), Tailscale Funnel.
7. In Meta App → WhatsApp → Configuration → Webhooks: set Callback URL to `https://your-domain/whatsapp/webhook`, paste the Verify Token, subscribe to `messages` field.
8. Restart the bot. Test by sending a message from your verified WhatsApp to the business number.
...
**Архитектура:** мост в `src/whatsapp/`. `handler.ts` не знает про baileys — принимает `(jid, text, sendReply)` и переиспользует `runAgent` + memory. Рядом лежат два адаптера: `baileys.ts` (неофициальный WebSocket) и `meta.ts` (официальный Meta Cloud API).

**Переключение на Meta Cloud API (после бизнес-верификации):** поставить `WHATSAPP_PROVIDER=meta` + заполнить `WHATSAPP_META_ACCESS_TOKEN`, `WHATSAPP_META_PHONE_NUMBER_ID`, `WHATSAPP_META_VERIFY_TOKEN`, `WHATSAPP_META_APP_SECRET`. Бот поднимет HTTP webhook на `WHATSAPP_META_WEBHOOK_PORT` (по дефолту 3001, путь `/whatsapp/webhook`).

**Шаги настройки Meta:**
1. Верифицировать бизнес в [Meta Business Manager](https://business.facebook.com). Занимает дни — планируй заранее.
2. Создать app на [developers.facebook.com](https://developers.facebook.com) → добавить продукт **WhatsApp**.
3. На странице WhatsApp → Getting Started: скопировать **Phone number ID** и сгенерировать **постоянный access token** (через System User, не временный на 24 часа).
4. Скопировать **App Secret** из App Settings → Basic.
5. Придумать случайный **Verify Token** (любая строка — это не от Meta, ты сам вставляешь одно и то же в `.env` и в UI Meta).
6. Поставить **HTTPS reverse proxy** перед ботом — Meta не разговаривает с plain HTTP. Варианты: Cloudflare Tunnel (бесплатно, без домена), nginx + Let's Encrypt (нужен домен), Tailscale Funnel.
7. В Meta App → WhatsApp → Configuration → Webhooks: задать Callback URL `https://твой-домен/whatsapp/webhook`, вставить Verify Token, подписаться на поле `messages`.
8. Рестарт бота. Тест: с верифицированного WhatsApp-номера отправь сообщение на бизнес-номер.
